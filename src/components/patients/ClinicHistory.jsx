...
const ClinicHistory = ({ data }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await axios
        ...
        .put(
          `${backendURL}/api/v1/clinic/uploadClinicHistory/${data._id}`,
          formData,
          {
  ...
  const handleDelteClinicHistory=async(e)=>{
    await axios.put(
      `${backendURL}/api/v1/clinic/deleteClinicHistory/${data._id}`,
      {historyUrl:e},
      {
        withCredentials: "include",
      }
  ...
  return (
    ...
    {isToastActive && (
        <Toast
          message={"Historia clinica subida exitosamente"}
          closeToast={handleCloseToast}
        />
      )}
          {isToastActiveDeletedCH && (
        <Toast
          message={"Historia clinica borrada exitosamente"}
...
<div className="py-8 px-2 grid gap-4 grid-cols-1 md:grid-cols-2">
      ...
      <div className=" mx-auto">
        ...
        >
          ...
          <strong className="font-medium text-gray-800 ">
            subir tu historia clínica
          </strong>
          o usar nuestro{" "}
          <strong className="font-medium text-gray-800 ">
            generador de historias clínicas
          </strong>{" "}
          con un formato general. La historia clínica que generes mediante
          nuestro formato general, se mantendra actualizado y puede ser
          descargado en pdf con el boton azul en la parte derecha inferior.
        </div>
        <form
      ...
      <div className="relative pt-4  overflow-x-auto shadow-md  sm:rounded-lg">
        <h2 className="p-2 text-center text-gray-600 ">
          Repositorio de historias clínicas subidas
        </h2>
        <table className="w-full  text-sm text-left text-gray-500 ">
          ...
          <tbody>
            {data.clinicHistory.map((e,index)=>(
               <tr>
 
               <th scope="col" className="px-6 py-3">
                 Historia clinicia {index+1}
               </th>
               ...
               <th scope="col" className="px-6 py-3">
                <button onClick={() => handleDelteClinicHistory(e)} className="bg-gray-200 text-gray-700 rounded-md p-2 hover:bg-gray-300" >Eliminar</button>
               </th>
             </tr>
