...
export default function SideBarComponent({
  ...
  const settingsButtons = [
    {
      name: "Consultorio",
      to: "/dashboard/perfil-consultorio",
      icon: ApartmentIcon,
      key: 5,
    },
    {
  ...
  return (
    <div
      className={` z-20 h-full rounded-lg  fixed   flex flex-col mt-4 left-0 w-14    ${
        isMobile ? " px-2 rounded-lg  " : "px-4 rounded-lg   "
      }`}
    >
      <Sidebar
      ...
      >
        ...
        {isMobile ? null : (
          <h1 className="px-4 text-gray-700 text-lg text-center font-bold my-4">
            {!clinicInfo
            ...
            {principalButtons.map((button) => (
              <div className="relative" key={button.key}>
                <MenuItem
                  ...
                  className={`  my-0.5 w-full  hover:bg-blue-400 hover:text-white rounded-lg  ${
                    selectedMain === button.to
                ...
                {selectedMain === button.name && (
                  <span
                    className={`absolute  right-0 top-0 bottom-0 h-full py-2 w-1.5 bg-[#3b68bd]`}
                  ></span>
                )}
              </div>
            ))}
            ...
            {settingsButtons.map((button) => (
              <div className="relative" key={button.key}>
                <MenuItem
                  style={{ backgroundColor: "transparent" }}
                  onClick={() => {
                    setSelectedSettings(button.to);
                    setSelectedMain("");
                  }}
                  className={` my-0.5  hover:bg-blue-400 hover:text-white rounded-lg  ${
                    selectedSettings === button.to
                      ? "text-blue-500 bg-blue-100"
                      : "text-gray-400"
                  }`}
                  icon={<button.icon className="mr-4" />}
        ...
        {!isMobile && (
          <Link href={"/"}>
            <h2 className="text-blue-500 text-center p-2">Alpha Clinicas</h2>
          </Link>
        )}
      </Sidebar>
    </div>
