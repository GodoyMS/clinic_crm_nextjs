...
export default function RegisterComponent(){
  ...
  console.log(infoClinic)
    ...
    useEffect(() => {
        ...
        if (infoClinic) router.push('/dashboard')
...
<form onSubmit={handleSubmit(submitForm)}  className="bg-white text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden max-w-5xl" >

{isModalRegisterActive && (
                  <div style={{zIndex:500}} className="fixed  flex items-center top-0 left-0 right-0 z-50 bg-black bg-opacity-50  w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-screen max-h-full">
                  <div className="relative w-full max-w-2xl mx-auto max-h-full ">
                      <div className="relative bg-white rounded-lg shadow ">
                          <div className="flex items-start justify-between p-4 border-b rounded-t ">
                              
                              <button onClick={()=>setIsModalRegisterActive(false)} className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center " >
                          ...
                          <div class="flex items-center p-6 space-x-2 border-t rounded-b ">
                            <Link  className='bg-blue-600 text-white hover:bg-blue-700 rounded-lg p-3' href={"/login"}>Iniciar sesión</Link>
                              <button onClick={()=>setIsModalRegisterActive(false)} className="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 ">Cerrar</button>
                          </div>
                      </div>
                  </div>
              </div>
            ...
            <div>
                <div className="flex -mx-3">
                    <div className="w-full px-3 mb-5">
                        <label htmlFor="" className="text-xs font-semibold px-1">Nombres y Apellidos</label>
                        <div className="flex">
                            <div className="input-form w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                            <input required type="text" {...register('username')} className="w-full  bg-gray-50 -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="John"/>
                        </div>
                    </div>
                ...
                <div className="flex -mx-3">
                    <div className="w-full px-3 mb-5">
                        ...
                        <div className="flex">
                            ...
                            <input required type="email"  {...register('email')}  className="form-input  bg-gray-50  w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="johnsmith@example.com"/>
                        </div>
                    </div>
                </div>
                <div className="flex -mx-3">
                    <div className="w-full px-3 mb-4">
                        ...
                        <div className="flex">
                            ...
                            type="password" 
                            {...register('password')}
                            required  className="w-full form-input -ml-10 pl-10 pr-3 bg-gray-50  py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="************"/>
                        </div>
                    </div>
                </div>
                <div className="flex -mx-3">
                    <div className="w-full px-3 mb-12">
                        <label htmlFor="" className="text-xs font-semibold px-1">Confirmar contraseña</label>
                        <div className="flex">
                            <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-lock-outline text-gray-400 text-lg"></i></div>
                            <input  type='password'
                              {...register('confirmPassword')}
                            required 
                            className="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 bg-gray-50  border-gray-200 outline-none focus:border-indigo-500 form-input" placeholder="************"/>
                        </div>
                    </div>
                </div>
                <div className="flex -mx-3">
                    <div className="w-full px-3 mb-5">
                        <button  type='submit'  className="block w-full button max-w-xs mx-auto bg-blue-500 hover:bg-blue-700 focus:bg-blue-700 text-white rounded-lg px-3 py-3 font-semibold">Registrarse</button>
                    </div>
                </div>
                <div className="flex -mx-3">
                    <div className="w-full text-center text-sm px-3 mb-5">
                       ¿Ya tienes una cuenta? <Link className='text-blue-500 hover:text-blue-600' href={"/login"}>Inicia sesion</Link>
                    </div>
                </div>
            </div>
        </div>
